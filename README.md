...
| 닉네임 변경 | PUT /api/user/info | 미검증 (테스트 중 서버 종료) |
...
WSL 안에 설치돼 있습니다. PowerShell에서 `wsl`로 진입한 뒤 `sudo` 없이 실행합니다.

```bash
redis-server --bind 0.0.0.0 --protected-mode no \
             --daemonize yes --dir /tmp --pidfile /tmp/redis.pid
```

`redis-cli ping`이 `PONG`을 내면 성공입니다. `exit`으로 나가도 계속 살아 있습니다.
...
아직 남아 있는 것과, 손대려면 어디를 봐야 하는지 정리했습니다.

| 항목 | 내용 | 해결 방향 |
| --- | --- | --- |
| 토큰 재발급 | `POST /api/token`이 refreshToken을 본문으로 받는데 그 토큰은 httpOnly 쿠키라 JS가 읽을 수 없음 | 백엔드가 쿠키에서 읽도록 변경 후 `http.js`에 401 재시도 추가 |
| 편지 제목 | `Letter` 엔티티에 제목 컬럼이 없어 목록에서 본문 앞 40자를 잘라 씀 | 엔티티에 `title` 추가 후 작성 폼과 카드에 반영 |
| 일기 소유자 검사 | `PUT`·`DELETE /api/diary/{id}`에 소유자 확인이 없음. id만 알면 남의 일기를 건드릴 수 있음 | 편지에 적용한 것처럼 `findByDiaryIdAndUser_UserId`로 조회 |
| Redis 설정 무효 | `application.yml`의 `jwt.redis.host/port`는 읽히지 않음. Spring Data Redis는 `spring.data.redis.*`를 봄 | 지금은 기본값 `localhost:6379`와 우연히 일치해 동작함. 호스트를 옮기면 문제가 됨 |
| 배포 트리거 | 워크플로가 `push: branches: ["dev"]`이고 작업 브랜치가 `dev`임 | push 시 EC2 배포가 실행됨. 원치 않으면 `workflow_dispatch`로 변경 |
| 테스트 데이터 | 검증용 `testuser1` 계정과 일기·편지 각 1건이 로컬 DB에 남아 있음 | 필요 시 삭제 |

---

EmoLetter 개발 핸드북 · 프론트엔드 63개 파일 2,272줄 기준 · 최종 확인 2026년 8월 23일
